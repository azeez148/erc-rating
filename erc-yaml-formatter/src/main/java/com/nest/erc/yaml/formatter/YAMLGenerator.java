package com.nest.erc.yaml.formatter;
...
import com.nest.erc.yaml.formatter.snakeyaml.emitter.Emitter;
...
public class YAMLGenerator extends GeneratorBase
{
	...
	protected final static Pattern ERC_NUMBER = Pattern.compile("^[-+]?\\d+(\\.\\d+)?");
	...
	private final static Set<String> MUST_QUOTE_VALUES = new HashSet<>(Arrays.asList(
			...
			"true", "True", "TRUE", "false", "False", "FALSE",
			"on", "On", "ON", "off", "Off", "OFF",
			"null", "Null", "NULL", "{{ riskType.label }} {{ number }}"
			));
	...
	protected final com.nest.erc.yaml.formatter.snakeyaml.DumperOptions.Version _docVersion;
	...
	public YAMLGenerator(IOContext ctxt, int jsonFeatures, int yamlFeatures,
			...
			com.nest.erc.yaml.formatter.snakeyaml.DumperOptions.Version version)
					throws IOException
	{
	...
	public void writeString(String text) throws IOException,JsonGenerationException
	{
		...
		if(text.contains("if IRPMApply == ")) {
			//System.out.println("Theft Of Clients' Property Coverage Limit - $5,000 unless otherwise indicated");
		}
		...
		if (Feature.MINIMIZE_QUOTES.enabledIn(_formatFeatures)) {
			...
			if (_valueNeedsQuoting(text)
				...
				style = STYLE_LITERAL;
			} else if(ERC_NUMBER.matcher(text).matches()) {
				style = STYLE_SCALAR.SINGLE_QUOTED;
			} else {
				style = STYLE_PLAIN;
			}
