...
import com.nest.erc.parser.input.InputLocationResolver;
import com.nest.erc.parser.input.ResolvedInputLocation;
...
public class ERCParser implements CommandLineRunner {
    ...
    @Autowired
    InputLocationResolver inputLocationResolver;

    public void generateERCProject() {

        try (ResolvedInputLocation resolvedLocation
                = inputLocationResolver.resolve(projectProperties.getInputFilelocation())) {
            System.out.println("Working directory: " + System.getProperty("user.dir"));
            Path localInputPath = resolvedLocation.getLocalDirectory();
            baseFolder = localInputPath.toString();
            projectProperties.setInputFilelocation(baseFolder);

            System.out.println("Input directory: " + resolvedLocation.getDescription());
            System.out.println("Absolute path: " + localInputPath.toAbsolutePath());

            File[] algFiles = getAlgFiles(baseFolder);
            if (algFiles.length == 0) {
                throw new IllegalStateException("No XML input files found under " + baseFolder);
            }
    ...
    /**
     * This method is used to keep an external json file for erc contents
     * parsing details.
     *
     * @param lob
     * @param state
